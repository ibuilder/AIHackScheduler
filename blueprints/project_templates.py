    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not all([project_name, project_location, start_date_str]):
                ...
                return render_template(
                    "projects/create_from_template.html", template=template, template_id=template_id
                )
            ...
            try:
                ...
                for template_task in template.get("tasks", []):
                    ...
                    task_start_offset = (
                        template_task["start_date"] - template.get("tasks")[0]["start_date"]
                    ).days
                        ...
                        for dep_name in template_task["dependencies"]:
                            ...
                            if current_task and dependency_task:
                                ...
                                if current_task.start_date <= dependency_task.end_date:
                                    current_task.start_date = dependency_task.end_date + timedelta(
                                        days=1
                                    )
                                    current_task.end_date = current_task.start_date + timedelta(
                                        days=current_task.duration
                                    )
                ...
                audit_logger.log_project_action(
                    "project_created_from_template",
                    project.id,
                    {"template_id": template_id, "task_count": len(task_map)},
                )
...
def api_template_detail(template_id):
    ...
    try:
        ...
        return jsonify({"template": template, "metrics": metrics, "success": True})
    except ValueError as e:
...
def api_estimate_project(template_id):
    ...
    try:
        ...
        return jsonify({"error": "Estimation failed", "success": False}), 500
...
def my_templates():
    ...
    projects = (
        Project.query.filter_by(company_id=current_user.company_id, created_by=current_user.id)
        .filter(Project.template_used.isnot(None))
