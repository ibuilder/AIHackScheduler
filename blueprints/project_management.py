...
from models import Project, ScheduleType, Task, TaskStatus
...
def dashboard_stats():
    ...
    try:
        stats = {
            ...
            "total_tasks": Task.query.join(Project)
            .filter(Project.company_id == current_user.company_id)
            .count(),
            "completed_tasks": Task.query.join(Project)
            .filter(
                Project.company_id == current_user.company_id, Task.status == TaskStatus.COMPLETED
            )
            .count(),
        }
