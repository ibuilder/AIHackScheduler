...
from models import Project, Resource, ResourceAssignment, Task, TaskStatus
...
def dashboard():
    ...
    # Get task statistics
    total_tasks = (
        Task.query.join(Project).filter(Project.company_id == current_user.company_id).count()
    )

    completed_tasks = (
        Task.query.join(Project)
        .filter(Project.company_id == current_user.company_id, Task.status == TaskStatus.COMPLETED)
        .count()
    )
    ...
    for project in projects[:10]:  # Top 10 projects
        ...
        if tasks:
            ...
            project_progress.append({"name": project.name, "progress": round(progress, 2)})
    ...
    status_distribution = [
        {"status": status.name, "count": count} for status, count in status_counts
    ]
...
def project_report(project_id):
    ...
    overdue_tasks = len(
        [
            t
            for t in tasks
            if t.end_date < datetime.now().date()
    ...
    overall_progress = (
        (sum(task.progress for task in tasks) / total_tasks) if total_tasks > 0 else 0
    )
    ...
    for resource in resources:
        ...
        utilization = (
            (total_assigned / resource.total_quantity * 100) if resource.total_quantity > 0 else 0
        )
    ...
    on_time_tasks = len(
        [t for t in tasks if t.status == TaskStatus.COMPLETED and t.updated_at.date() <= t.end_date]
    )
    late_tasks = len(
        [t for t in tasks if t.status == TaskStatus.COMPLETED and t.updated_at.date() > t.end_date]
    )
