...
def financial_dashboard():
    ...
    recent_transactions = (
        Transaction.query.filter_by(company_id=current_user.company_id)
...
def invoice_list():
    ...
    stats = {
        ...
        .scalar()
        or 0,
        "outstanding_amount": db.session.query(func.sum(Invoice.total_amount - Invoice.paid_amount))
        .filter(
            Invoice.company_id == current_user.company_id,
            Invoice.status.in_([InvoiceStatus.SENT, InvoiceStatus.VIEWED, InvoiceStatus.PARTIAL]),
        )
...
def generate_transaction_number(company_id):
    ...
    last_transaction = (
        Transaction.query.filter(
            Transaction.company_id == company_id, Transaction.transaction_number.like(f"{prefix}%")
        )
...
def generate_invoice_number(company_id):
    ...
    last_invoice = (
        Invoice.query.filter(
            Invoice.company_id == company_id, Invoice.invoice_number.like(f"{prefix}%")
        )
...
def financial_dashboard_stats():
    ...
    try:
        ...
        for month in range(1, 13):
            revenue = (
                get_total_revenue(current_user.company_id, current_year)
                if month <= current_month
                else 0
            )
            expenses = (
                get_total_expenses(current_user.company_id, current_year)
                if month <= current_month
                else 0
            )
            monthly_data.append(
