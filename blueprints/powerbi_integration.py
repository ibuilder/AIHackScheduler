...
from models import PowerBIIntegration, Project
...
class PowerBIClient:
    ...
    def get_datasets(self, workspace_id):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/groups/{workspace_id}/datasets", headers=headers
            )
            response.raise_for_status()
            return response.json()
    ...
    def execute_query(self, workspace_id, dataset_id, dax_query):
        ...
        query_data = {"queries": [{"query": dax_query}]}
    ...
    try:
        ...
        for query_result in result.get("results", []):
            ...
            for table in tables:
                ...
                for row in rows:
                    project_data = dict(
                        zip([col["name"] for col in table.get("columns", [])], row, strict=False)
                    )
                    ...
                    if existing_project:
                        ...
                        try:
                            ...
                            logging.warning(
                                f"Invalid date format in Power BI data for project {project_data.get('ProjectName')}"
                            )
...
def test_connection():
    ...
    try:
        ...
        if not all([client_id, client_secret, tenant_id]):
            return jsonify({"success": False, "error": "Power BI credentials not configured"})
        ...
        if not token:
            return jsonify({"success": False, "error": "Failed to authenticate with Power BI"})
...
def sync_status():
    ...
    try:
        recent_syncs = (
            PowerBIIntegration.query.filter_by(company_id=current_user.company_id)
            .order_by(PowerBIIntegration.sync_timestamp.desc())
            .limit(10)
            .all()
        )
        ...
        return jsonify({"success": True, "sync_history": sync_history})
