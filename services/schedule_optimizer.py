...
def _relation_type(raw) -> RelationType:
    try:
...
class ScheduleOptimizer:
    ...
    def optimize_project_schedule(
        self, project_id: int, optimization_type: str = "time"
    ) -> dict[str, Any]:
        ...
        if not tasks:
            return {"success": False, "error": "No tasks found for optimization"}

        # Optimising an unsound network produces confident nonsense, so the
        # schedule has to clear the blocking DCMA checks first.
        from services.schedule_analysis import health_check

        health = health_check(project_id)
        if health.get("success") and not health.get("optimisable", True):
            blocking = [c["name"] for c in health["checks"] if c["status"] == "fail"]
            return {
                "success": False,
        ...
        return {
            ...
            "schedule_grade": health.get("grade"),
            "results": results,
    ...
    def _optimize_for_time(self, project: Project, tasks: list[Task]) -> dict[str, Any]:
        ...
        return {
            ...
            "optimized_duration_days": max(
                1, (project.end_date - project.start_date).days - potential_savings
            ),
        }
    ...
    def _optimize_for_cost(self, project: Project, tasks: list[Task]) -> dict[str, Any]:
        ...
        resource_assignments = (
            ResourceAssignment.query.join(Task).filter(Task.project_id == project.id).all()
        )
    ...
    def _find_critical_path(self, tasks: list[Task]) -> list[Task]:
        ...
        Delegates to :mod:`core.cpm`, which orders the forward and backward
        passes topologically and handles SS/FF/SF logic. The earlier inline
        version walked tasks in list order, so a predecessor declared after
        its successor was read before it had been computed — producing a
        critical path that silently depended on row ordering.
        """
        if not tasks:
            return []
        ...
        activities = [
            Activity(id=str(task.id), name=task.name, duration=task.duration or 0) for task in tasks
        ]
        relationships = [
            Relationship(
            ...
            if dep.predecessor_task_id in task_dict and dep.task_id in task_dict
        ]

        try:
            result = calculate_cpm(activities, relationships)
        except (ScheduleCycleError, ValueError):
            # A broken network has no meaningful critical path. Callers get an
            # empty result rather than a fabricated one.
            return []

        chain = longest_path(result, relationships)
        return [task_dict[int(aid)] for aid in chain if int(aid) in task_dict]
    ...
    def _find_parallelization_opportunities(self, tasks: list[Task]) -> list[dict]:
        ...
        for i, task1 in enumerate(tasks):
            for task2 in tasks[i + 1 :]:
                ...
                if (
                    task1.id not in task2_deps
                    and task2.id not in task1_deps
                    and not task1_deps.intersection(task2_deps)
                ):
    ...
    def _optimize_critical_path_resources(
        self, critical_tasks: list[Task], all_tasks: list[Task]
    ) -> list[dict]:
        ...
        for critical_task in critical_tasks:
            ...
            for assignment in resource_assignments:
                ...
                other_assignments = (
                    ResourceAssignment.query.join(Task)
                    .filter(
    ...
    def _optimize_resource_utilization(
        self, resources: list[Resource], assignments: list[ResourceAssignment]
    ) -> list[dict]:
                    ...
                    optimizations.append(
                        {
                            ...
                            "cost_saved": (resource.total_quantity - total_assigned * 1.1)
                            * resource.unit_cost
                    ...
                    optimizations.append(
                        {
                            ...
                            "additional_cost": (total_assigned * 1.2 - resource.total_quantity)
                            * resource.unit_cost
                            if resource.unit_cost
                            else 0,
                            "impact": "high",
    ...
    def _optimize_task_scheduling_for_cost(
        self, tasks: list[Task], resources: list[Resource]
    ) -> list[dict]:
    ...
    def _find_resource_substitutions(
        self, resources: list[Resource], assignments: list[ResourceAssignment]
    ) -> list[dict]:
        ...
        for resource in high_cost_resources:
            ...
            similar_resources = [
                ...
                and r.unit_cost
                and r.unit_cost < resource.unit_cost * 0.8
            ]
    ...
    def _optimize_resource_leveling(
        self, tasks: list[Task], resources: list[Resource]
    ) -> list[dict]:
    ...
    def _recommend_resource_adjustments(
        self, resources: list[Resource], tasks: list[Task]
    ) -> list[dict]:
