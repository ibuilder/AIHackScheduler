...
class FabricService:
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to get Fabric access token: {e}") from e
    ...
    def _make_api_request(self, endpoint: str, method: str = "GET", data: dict = None) -> dict:
        ...
        try:
            ...
            raise RuntimeError(f"Fabric API request failed: {e}") from e
    ...
    def sync_project_data(self, project_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                self._make_api_request(f"datasets/{dataset_name}")
                # Dataset exists, update it
                result = self._make_api_request(
                    f"datasets/{dataset_name}/data", method="PUT", data=fabric_data
                )
            except Exception:
                ...
                result = self._make_api_request("datasets", method="POST", data=dataset_config)

            return {
                "success": True,
                "dataset_name": dataset_name,
                "records_synced": {"tasks": len(tasks), "resources": len(resources)},
                "sync_timestamp": datetime.utcnow().isoformat(),
    ...
    def get_project_analytics(self, project_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            analytics_result = self._make_api_request(
                f"datasets/{dataset_name}/query", method="POST", data=analytics_query
            )
            ...
            return {"success": False, "error": f"Analytics query failed: {str(e)}"}
    ...
    def create_data_pipeline(self, project_id: int, pipeline_config: dict) -> dict[str, Any]:
        ...
        try:
            result = self._make_api_request(
                "dataPipelines", method="POST", data=pipeline_definition
            )
            ...
            return {"success": False, "error": f"Pipeline creation failed: {str(e)}"}
