...
def process_project_file(self, project_id, file_path, file_type):
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS", meta={"current": 25, "total": 100, "status": "Reading file..."}
        )
        ...
        self.update_state(
            state="PROGRESS", meta={"current": 50, "total": 100, "status": "Creating tasks..."}
        )
        ...
        return {"status": "completed", "tasks_created": len(created_tasks), "file_path": file_path}

    except Exception as exc:
        self.update_state(state="FAILURE", meta={"current": 0, "total": 100, "status": str(exc)})
        raise exc


@celery_app.task(bind=True)
def sync_azure_services(self, project_id, services=None):
    """Sync project data with Azure services."""
    if services is None:
        services = ["ai", "fabric", "foundry"]
    try:
        from extensions import db
        ...
        for i, service in enumerate(services):
            ...
            try:
                ...
                integration = AzureIntegration.query.filter_by(
                    project_id=project_id, service_type=service
                ).first()
...
def send_notification(user_id, notification_type, data):
    ...
    try:
        ...
        return {"status": "completed", "notification": notification_data}
            ...
            tasks.append(
                {
                    ...
                    else datetime.strptime(str(row[2]), "%Y-%m-%d").date(),
