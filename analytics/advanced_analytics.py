...
from models import AuditLog, Project, Task, TaskStatus, User
...
class AdvancedAnalytics:
    ...
    def get_project_performance_metrics(self, company_id, date_range=30):
        ...
        end_date = date.today()
        end_date - timedelta(days=date_range)
        ...
        metrics["budget_performance"] = {
            ...
            "variance_percent": ((total_actual - total_budget) / total_budget * 100)
            if total_budget > 0
            else 0,
        ...
        metrics["schedule_performance"] = {
            ...
            "schedule_adherence_percent": (on_time_projects / len(projects) * 100)
            if projects
            else 0,
        }
    ...
    def get_task_analytics(self, company_id, project_id=None):
        ...
        analytics = {
            ...
            "completion_rate": (status_counts.get("COMPLETED", 0) / len(tasks) * 100)
            if tasks
            else 0,
            "average_duration_days": round(avg_duration, 1),
            "completion_trends": completed_tasks_by_week,
            "overdue_tasks": len(
                [t for t in tasks if t.end_date < date.today() and t.status != TaskStatus.COMPLETED]
            ),
    ...
    def get_user_productivity_metrics(self, company_id):
        ...
        return {
            ...
            "active_users_30_days": len(
                [
                    u
                    for u in users
                    if u.last_login and u.last_login >= datetime.now() - timedelta(days=30)
                ]
            ),
    ...
    def _get_completion_trends(self, tasks):
        ...
        for task in tasks:
            if task.status == TaskStatus.COMPLETED:
                ...
                week = (
                    task.updated_at.strftime("%Y-W%U")
                    if task.updated_at
                    else datetime.now().strftime("%Y-W%U")
                )
