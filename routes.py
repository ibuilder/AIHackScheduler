...
from models import Project, Task, TaskStatus, UserRole
...
def dashboard():
    ...
    try:
        ...
        status_counts = {}
        all_tasks = (
            Task.query.join(Project).filter(Project.company_id == current_user.company_id).all()
        )
        ...
        # `app` was never imported here, so any dashboard query error raised a
        # NameError from inside the handler instead of being logged.
        current_app.logger.error(f"Dashboard data calculation error: {str(e)}")
        project_progress = []
        status_distribution = []
        overdue_tasks = 0
